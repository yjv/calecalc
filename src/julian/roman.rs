...
pub fn roman_from_fixed(date: RD) -> Roman {
    let Julian { year, month, day } = julian_from_fixed(date);
    ...
    if day == 1 {
        ...
        Roman { year: adjusted_year, month: next_month, event: Event::Kalends, count: kalends - date + 1, leap: false }
    } else if day < 25 {
